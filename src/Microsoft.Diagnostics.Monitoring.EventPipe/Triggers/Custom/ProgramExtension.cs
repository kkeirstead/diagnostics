...
namespace Microsoft.Diagnostics.Monitoring.EventPipe.Triggers.Custom
{
  ...
  internal partial class ProgramExtension //: IExtension, IEgressExtension
  {
    ...
    /// <inheritdoc/>
    public async Task<string> EgressArtifact(ExtensionTriggerPayload configPayload, Func<Stream, CancellationToken, Task> getStreamAction, CancellationToken token)
    {
      if (!Declaration.SupportedExtensionTypes.Contains("Trigger"))
      {
      ...
      {
        Console.WriteLine("Prog Info didn't exist...");
      ...
      pStart.ArgumentList.Add("Trigger");
      pStart.ArgumentList.Add(configPayload.Args); // Make this into args list
      ...
      using OutputParser<string> parser = new(p, _logger);

      //_logger.ExtensionStarting(pStart.FileName, pStart.Arguments);
      if (!p.Start())
      {
          Console.WriteLine("Uh oh...process couldn't start"); // Put something real here
          //ExtensionException.ThrowLaunchFailure(_extensionName);
      }

      parser.BeginReading();

      await JsonSerializer.SerializeAsync<ExtensionTriggerPayload>(p.StandardInput.BaseStream, configPayload, options: null, token);
      ...
      string result = await parser.ReadResult();
